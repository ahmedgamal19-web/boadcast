...
# ==========================================
# تقسيم النص إلى أجزاء بناءً على عدد الكلمات
# ==========================================
def split_text_into_chunks(text, max_words_per_chunk=800):
    """
    تقسم النص إلى أجزاء كل جزء يحتوي على max_words_per_chunk كلمة تقريباً،
    مع الحفاظ على سلامة الجمل.
    """
    words = text.split()
...
def generate_script(text, model):
    ...
    for idx, chunk in enumerate(chunks):
        # إضافة سياق للأجزاء اللاحقة
        if idx == 0:
            context = ""
        else:
            context = "هذا هو الجزء التالي من النص، استمر في الحوار بنفس الأسلوب مع تجنب تكرار الأسئلة."
        
        prompt = f"""
أنت منتج بودكاست تعليمي متميز، على مستوى عالٍ من الاحترافية، مثل بودكاست "NotebookLM" من Google. مهمتك هي تحويل النص الأكاديمي التالي إلى حوار صوتي شيق وواضح بين أستاذ جامعي وطالب.

تعليمات دقيقة:
1. **اللغة**: العربية الفصحى الحديثة فقط، بدون أي كلمات إنجليزية. اكتب المصطلحات بالعربية مع شرح مبسط.
2. **التنسيق**: كل جملة تبدأ بـ "الأستاذ:" أو "الطالب:" متبوعة بنقطتين ومسافة.
3. **طبيعة الحوار**: الأستاذ يشرح بأسلوب قصصي، والطالب يسأل أسئلة ذكية.
4. **تجنب التكرار**: لا تكرر نفس السؤال أو الجواب، وتنوع في صياغة أسئلة الطالب (لا تبدأ دائماً بـ "هل يمكنك شرح..."، استخدم صيغاً متنوعة).
...
"""
        try:
            ...
            script_part = response['response'].strip()
...
def player_html(audio_b64, timestamps, dialogue):
    ...
    html = f"""
    <div>
        <audio id="player" controls style="width: 100%; border-radius: 50px; margin-bottom: 20px; background: #0f172a;">
        ...
        <div style="display: flex; gap: 10px; flex-wrap: wrap; justify-content: center; margin-bottom: 20px;">
            ...
            <button id="skip-fwd">+10s ⏩</button>
        </div>
        <div id="transcript" style="max-height: 560px; overflow-y: auto; background: rgba(0,0,0,0.2); border-radius: 36px; padding: 20px;">
            {bubbles}
        </div>
    </div>
    <script>
        ...
        document.getElementById('skip-back').onclick = () => player.currentTime = Math.max(0, player.currentTime - 10);
        document.getElementById('skip-fwd').onclick = () => player.currentTime = Math.min(player.duration, player.currentTime + 10);
        ...
        player.ontimeupdate = () => {{
            ...
            document.querySelectorAll('.bubble').forEach(b => {{
                ...
                let live = b.querySelector('.live-icon');
                if(live) live.style.opacity = '0';
            }});
            if(activeIndex !== -1) {{
                let activeBubble = document.querySelector(`.msg[data-index='${{activeIndex}}'] .bubble`);
                if(activeBubble) {{
                    activeBubble.style.boxShadow = '0 0 28px rgba(96,165,250,0.8)';
                    activeBubble.style.transform = 'scale(1.02)';
                    let live = activeBubble.querySelector('.live-icon');
    ...
    if st.button("🚀 إنتاج البودكاست", use_container_width=True):
        if not user_text.strip():
            ...
            with st.spinner("🤖 الذكاء الاصطناعي يصوغ الحوار..."):
                script_raw = generate_script(user_text, MODEL_NAME)
            if not script_raw:
                st.error("فشل توليد الحوار، تأكد من تشغيل Ollama")
            else:
                ...
                if len(dialogue) < 2:
                    ...
                    st.components.v1.html(player_html(b64, stamps, dialogue), height=720)
